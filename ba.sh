#!/bin/bash

_get_ziplink () {
    local regex
    regex='(https?):/github.com/Tythono/Tython90.git'
    if [[ $UPSTREAM_REPO == "Tython90" ]]
    then
        if [[ $GITHUB_TOKEN ]]
        then
            echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vVHl0aG9uOTAvYXJjaGl2ZS9tYWluLnppcA==" | base64 -d | sed "s|https://github.com|https://${GITHUB_TOKEN}@github.com|"
        else
            echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vVHl0aG9uOTAvYXJjaGl2ZS9tYWluLnppcA==" | base64 -d
        fi
    elif [[ $UPSTREAM_REPO =~ $regex ]]
    then
        if [[ $GITHUB_TOKEN ]]
        then
            local url_with_token
            url_with_token=$(echo "${UPSTREAM_REPO}" | sed "s|https://github.com|https://${GITHUB_TOKEN}@github.com|")
            if [[ $UPSTREAM_REPO_BRANCH ]]
            then
                echo "${url_with_token}/archive/${UPSTREAM_REPO_BRANCH}.zip"
            else
                echo "${url_with_token}/archive/main.zip"
            fi
        else
            if [[ $UPSTREAM_REPO_BRANCH ]]
            then
                echo "${UPSTREAM_REPO}/archive/${UPSTREAM_REPO_BRANCH}.zip"
            else
                echo "${UPSTREAM_REPO}/archive/main.zip"
            fi
        fi
    else
        if [[ $GITHUB_TOKEN ]]
        then
            echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vVHl0aG9uOTAvYXJjaGl2ZS9tYWluLnppcA==" | base64 -d | sed "s|https://github.com|https://${GITHUB_TOKEN}@github.com|"
        else
            echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vVHl0aG9uOTAvYXJjaGl2ZS9tYWluLnppcA==" | base64 -d
        fi
    fi
}

_get_repolink () {
    local regex
    local rlink
    regex='(https?):/github.com/Tythono/Tython90.git'
    if [[ $UPSTREAM_REPO == "Deploy" ]]
    then
        rlink=`echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vQWwtU2FybWFkaTI=" | base64 -d`
    elif [[ $UPSTREAM_REPO =~ $regex ]]
    then
        if [[ $GITHUB_TOKEN ]]
        then
            rlink=$(echo "${UPSTREAM_REPO}" | sed "s|https://github.com|https://${GITHUB_TOKEN}@github.com|")
        else
            rlink=`echo "${UPSTREAM_REPO}"`
        fi
    else
        rlink=`echo "aHR0cHM6Ly9naXRodWIuY29tL1R5dGhvbm8vQWwtU2FybWFkaTI=" | base64 -d`
    fi
    echo "$rlink"
}

_run_python_code() {
    python3${pVer%.*} -c "$1"
}

_run_catpack_git() {
    $(_run_python_code 'from git import Repo
import sys
OFFICIAL_UPSTREAM_REPO = "https://github.com/Tythono/Al-Sarmadi2.git"
ACTIVE_BRANCH_NAME = "main"
repo = Repo.init()
origin = repo.create_remote("temponame", OFFICIAL_UPSTREAM_REPO)
origin.fetch()
repo.create_head(ACTIVE_BRANCH_NAME, origin.refs[ACTIVE_BRANCH_NAME])
repo.heads[ACTIVE_BRANCH_NAME].checkout(True) ')
}

_run_cat_git() {
    local repolink=$(_get_repolink)
    $(_run_python_code 'from git import Repo
import sys
OFFICIAL_UPSTREAM_REPO="'$repolink'"
ACTIVE_BRANCH_NAME = "'$UPSTREAM_REPO_BRANCH'" or "main"
repo = Repo.init()
origin = repo.create_remote("temponame", OFFICIAL_UPSTREAM_REPO)
origin.fetch()
repo.create_head(ACTIVE_BRANCH_NAME, origin.refs[ACTIVE_BRANCH_NAME])
repo.heads[ACTIVE_BRANCH_NAME].checkout(True) ')
}

_set_bot () {
    local zippath
    zippath="main.zip"
    echo "⌭ جاري تنزيل اكواد السورس ⌭"
    wget -q $(_get_ziplink) -O "$zippath"
    echo "⌭ تفريغ البيانات ⌭"
    CATPATH=$(zipinfo -1 "$zippath" | head -1 | cut -d/ -f1)
    unzip -qq "$zippath"
    echo "⌭ تـم التفريـغ ⌭"
    echo "⌭ يتم التنظيف ⌭"
    rm -rf "$zippath"
    sleep 5
    _run_catpack_git
    cd $CATPATH
    _run_cat_git
    python3 ../setup/updater.py ../requirements.txt requirements.txt
    chmod -R 755 bin
    echo "⌭ جـاري بـدء تنصيـب تايثـون ⌭"
    echo ""
    python3 -m Tython
}

_set_bot
